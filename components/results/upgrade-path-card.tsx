    ...
    <div className="bg-white dark:bg-surface rounded-xl shadow-lg p-6">
      ...
      <div className="space-y-4">
        {upgrades.map((upgrade, i) => (
          <div key={i} className="border border-slate-200 dark:border-border rounded-lg p-4 hover:border-brand-400 transition">
              <div className="flex items-start justify-between mb-2">
                <div className="flex items-center gap-3">
                  <span className="inline-flex w-8 h-8 rounded-full bg-brand-50 dark:bg-surface text-brand-700 dark:text-brand-100 font-bold items-center justify-center">
                    {i + 1}
                  </span>
                  <span className="text-lg font-semibold text-slate-900 dark:text-slate-50">{upgrade.category}</span>
                </div>
                <span className="text-sm font-medium text-brand-700 dark:text-brand-200 bg-brand-50 dark:bg-surface px-3 py-1 rounded-full">
                  {upgrade.impactSummary}
                </span>
              </div>
